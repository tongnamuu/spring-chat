...
| 캡처 | 검증 상태 | 확인 내용 |
| --- | --- | --- |
| [기본 회원가입 폼](./chat-3-signup-form-desktop.png) | 회원가입 탭 진입 | 로그인/회원가입 분리, 일반 아이디·이메일 정책, nickname/password/확인 필드 안내 |
| [서버 검증 오류 상단](./chat-3-signup-validation-desktop-top.png) | 잘못된 username, nickname, password 제출 | 각 입력 필드 아래에 서버 검증 사유 표시, 모달 상단과 탭 유지 |
...
| [중복 username](./chat-3-signup-duplicate-desktop.png) | 이미 생성된 username 재사용 | HTTP 409 응답을 username 필드의 사용자용 오류로 표시 |
...
| 캡처 | 검증 상태 | 확인 내용 |
| --- | --- | --- |
| [모바일 필드 검증](./chat-3-signup-validation-mobile.png) | username/nickname/password 동시 오류 | 모달이 viewport 안에 유지되고 텍스트, 입력, 오류, 버튼이 겹치거나 잘리지 않음 |
