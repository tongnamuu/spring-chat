...
| Method | Endpoint | 설명 |
| :--- | :--- | :--- |
| `POST` | `/api/auth/signup` | 회원가입 (email, nickname, password, 성공 시 201) |
...
REST와 STOMP 모두 `CHAT_SESSION` HttpOnly 쿠키에서 사용자를 결정합니다. 세션 principal에는
변경되지 않는 내부 `userId`만 저장하며, 이메일은 쿠키·세션 principal·일반 사용자 응답에 포함하지 않습니다. 클라이언트가 보낸
...
email은 254자 이내의 유효한 이메일만 허용하고 소문자로 정규화해 로그인 식별자로 저장합니다.
password는 8~72자이면서 영문과 숫자를 각각 하나 이상 포함하고, nickname은 앞뒤 공백 없는 2~20자로 제한합니다.
